...
# versioning: enabled
# encryption: enabled
resource "aws_s3_bucket" "cicd-codepipeline-tfstate-backend" {
    bucket = "cicd-codepipeline-tfstate-backend"
    tags = {
      Name = "cicd-codepipeline-tfstate-backend"
    }
}
resource "aws_s3_bucket_acl" "tfstate-acl" {
    bucket = aws_s3_bucket.cicd-codepipeline-tfstate-backend.id
    acl = "private"
}
